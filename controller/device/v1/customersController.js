...
const addCustomers = async (req, res) => {
  try {
    ...
    let validate = validation.validateParamsWithJoi(
      dataToCreate,
    ...
    let createdResult = await dbService.createOne(Customers,dataToCreate);
    if (createdResult){
...
const bulkInsertCustomers = async (req, res)=>{
  try {
    ...
    if (dataToCreate !== undefined && dataToCreate.length){
      ...
      let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Customers,[ 'PhoneNumber' ],dataToCreate,'BULK_INSERT');
...
const updateCustomers = async (req, res) => {
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Customers,[ 'PhoneNumber' ],dataToUpdate,'UPDATE', query);
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
    }
...
const bulkUpdateCustomers = async (req, res)=>{
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Customers,[ 'PhoneNumber' ],dataToUpdate,'BULK_UPDATE', filter);
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
    }
...
const softDeleteCustomers = async (req, res) => {
  try {
    ...
    let result = await dbService.update(Customers, query,updateBody);
    if (!result){
      return res.recordNotFound();
    }
...
const deleteCustomers = async (req, res) => {
  const result = await dbService.deleteByPk(Customers, req.params.id);
  return  res.success({ data :result });
};
...
const deleteManyCustomers = async (req, res) => {
  try {
    ...
    let deletedCustomers = await dbService.destroy(Customers,query);
    return res.success({ data :{ count :deletedCustomers.length } });
  }
  catch (error){
    return res.internalServerError({ message:error.message });  
  }
...
const softDeleteManyCustomers = async (req, res) => {
  try {
    ...
    const options = {};
    let updatedCustomers = await dbService.update(Customers,query,updateBody, options);
    if (!updatedCustomers) {
      return res.recordNotFound();
    }
    return  res.success({ data :{ count: updatedCustomers.length } });
